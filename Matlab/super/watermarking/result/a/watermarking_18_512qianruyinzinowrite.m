...
% imwrite(uint8(round(J2)),strcat(int2str(a_num), '_watermakered.bmp'));
...
% imwrite(x_temp1,strcat(int2str(a_num), '_chazhi.bmp'));
x_temp2=x_temp1(:);
x_temp3=abs(x_temp2);
...
% imwrite(zhongkaiback, strcat(int2str(a_num),strcat(int2str(numsave), '.bmp')));
simsave(numsave)=sim;
PSNRsave(numsave)=PSNR;
end
a_PSNR(a_num)=PSNR;
a_sim(a_num)=sum(simsave)/12;
end
save a_sim a_sim
save a_PSNR a_PSNR