  ...
  for layer=1:3 
      for row=1:Imagesize(1)
            for col=1:Imagesize(2)
                ...
                w_b=y;
               Jiemi(row,col)=mod(m-(round(1000*u_a)+round(1000*w_b)),256);
               delta=Image(row,col)/256;
                   yc1=mod((yc1+delta),1);
                     xc1=xc1;
                for i_arnold=1:3 
                        abs=[xc1;yc1];
   ...
   for row=1:Imagesize(1)
     for col=1:Imagesize(2)
        ...
        y=b1;
          ...
          w_b=y;
               Jiemi(row,col)=mod(m*256-(round(1000*u_a)+round(1000*w_b)),256);
               delta=Image(row,col)/256;
               yc1=mod((yc1+delta),1);
               xc1=xc1;
                for i_arnold=1:3 
