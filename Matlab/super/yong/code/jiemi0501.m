   ...
   for row=1:Imagesize(1)
     for col=1:Imagesize(2)
        ...
        y=b1;
          ...
          w_b=y;
               EImage(row,col)=mod(m-(round(1000*u_a)-round(1000*w_b)),256);
               delta=EImage(row,col)/256;
               yc1=mod((yc1+delta),1);
               xc1=xc1;
                for i_arnold=1:3 
                        ans=[xc1;yc1];
...
save Encrypted EImage;
   EImage;
   UEImage=uint8(EImage);
figure,
imshow(Image),title('O Image');
figure(2),imshow(UEImage),title('EImage');
imwrite(UEImage,'ImageE.tif','tif');
toc;