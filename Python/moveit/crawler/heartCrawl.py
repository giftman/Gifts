...
class Retriever(object):#download Web pages
    ...
    def filename(self,url,deffile='index.html'):
        parsedurl = urlparse(url,'http:',0) ##parse path<Plug>(neosnippet_expand)
        ...
        if not isdir(ldir): #crete archive dir if nec.
    ...
    def download(self): #download Web page
        ...
        return retval
    def parseAndGetLinks(self):#parseHTML,save links
...
class Crawler(object):#manage entire crawling process
    ...
    def getPage(self,url):
       ...
       for eachLink in links:
            if eachLink[:4] != 'http' and find(eachLink,'://') == -1:eachLink = urljoin(url,eachLink)
            print '* ',eachLink,
